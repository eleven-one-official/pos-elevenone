...
// ---------------------------------------------------------------------------
// Configuration › Payment Methods — the payment journals the POS offers on
// its Payment screen. Odoo-style list + form over the real payment-methods
// API; deleting is selection-based like the pricelist list. Writes are
...
export default function PosPaymentMethods() {
  ...
  const deleteChecked = async () => {
    ...
    setActionError(null)
    try {
      for (const id of checked) await deletePaymentMethod(id)
      await load()
    } catch (e: unknown) {
  ...
  if (methods === null) {
    return <LoadingState label="Loading payment methods..." className="h-full" />
  }
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-neutral-200/80 px-4 pb-2.5 pt-4">
        <div className="flex flex-wrap items-start justify-between gap-x-10 gap-y-3">
          <div>
            <h1 className="text-xl text-neutral-700">Payment Methods</h1>
      ...
      <div className="overflow-y-auto">
            ...
            <tr className="border-b border-neutral-200 text-left text-neutral-800">
              <th className="w-10 px-4 py-2.5">
                <input
                  ...
                  checked={visible.length > 0 && visible.every((m) => checked.has(m.id))}
                  onChange={(e) =>
                    setChecked(e.target.checked ? new Set(visible.map((m) => m.id)) : new Set())
                  }
              ...
              <th className="w-[25%] py-2.5 pr-4 font-bold">Channel</th>
          ...
          <tbody>
            {visible.map((m) => (
              ...
              >
                ...
                <td className="py-2 pr-4 text-neutral-800">{m.label}</td>
                <td className="py-2 pr-4">
                  {PAY_CHANNELS.find((c) => c.value === m.channel)?.label ?? m.channel}
                </td>
                <td className="py-2 pr-4 text-right">{m.sort_order}</td>
                <td className="py-2 pr-4">
                  <span
        ...
        {visible.length === 0 && (
          <div className="p-10 text-center text-sm text-neutral-500">
            {query.trim()
              ? `No payment method matches "${query}".`
              : 'No payment methods yet — hit Create to add the first one.'}
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-[3px] border border-neutral-200 bg-white shadow-xl">
            ...
            <p className="px-5 py-4 text-sm text-neutral-700">
              ...
              {checked.size === 1 ? 'this payment method' : `these ${checked.size} payment methods`}
              ? The POS payment screen stops offering {checked.size === 1 ? 'it' : 'them'}{' '}
              immediately.
            </p>
...
// ---------------------------------------------------------------------------
// Create/edit form — label, channel, sequence and the active toggle.
// ---------------------------------------------------------------------------
...
function PaymentMethodForm({
  ...
  const [channel, setChannel] = useState(method?.channel ?? PAY_CHANNELS[0].value)
  const [isActive, setIsActive] = useState(method?.is_active ?? true)
  const [sortOrder, setSortOrder] = useState(method ? String(method.sort_order) : '')
  ...
  const save = async () => {
    if (saving) return
    if (!label.trim()) {
      setError('The method name is required.')
      return
    }
    ...
    setError(null)
    try {
      if (method) await updatePaymentMethod(method.id, input)
      else await createPaymentMethod(input)
      await onSaved()
    } catch (e: unknown) {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-neutral-200/80 px-4 pb-2.5 pt-4">
        <div className="truncate text-[15px] text-neutral-700">
          ...
          <span>{method ? method.label : 'New'}</span>
        </div>
        <div className="mt-2 flex items-center gap-1.5">
          <button
            type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-neutral-100/60 pb-6">
        ...
        <div className="mx-4 mt-4 rounded-[2px] border border-neutral-200 bg-white px-8 py-6 shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
          <div className="text-[13px] font-bold text-neutral-800">Method</div>
          <input
          ...
          <div className="mt-6 grid grid-cols-1 gap-x-16 gap-y-3 xl:grid-cols-2">
            <FieldGroup>
              ...
              <input
                ...
                placeholder="Position on the payment screen"
          ...
          <p className="mt-8 border-t border-neutral-200 pt-4 text-[12.5px] italic text-neutral-500">
            Active methods appear on the POS payment screen in sequence order. The channel decides
            how the recorded payment is booked (cash, ABA QR, KHQR or card).
          </p>
        </div>
      </div>
    </div>
